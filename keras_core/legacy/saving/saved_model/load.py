...
def load(path, compile=True, options=None):
    ...
    # TODO(nkovela): Debug initialization of variables for graph modes
    if not tf.executing_eagerly():
        sess = tf.compat.v1.get_default_session()  # Variables are initialized by this call.
        sess.run(
...
class KerasObjectLoader:
    ...
    def del_tracking(self):
        ...
        for node in self.loaded_nodes.values():
            node = node[0]
            if not isinstance(node, layers.Layer):
            ...
            if isinstance(node, models.Functional):
    ...
    def _add_children_recreated_from_config(self, obj, proto, node_id):
        ...
        if isinstance(obj, layers.Layer) and not obj.built:
    ...
    def _revive_graph_network(self, identifier, metadata, node_id):
        ...
        # The model is a custom Sequential model.
            ...
            model = models.Sequential(name=class_name)
        else:
    ...
    def finalize_objects(self):
        ...
        for node_id, (node, _) in self.loaded_nodes.items():
            if (
                not isinstance(node, layers.Layer)
    ...
    def _unblock_model_reconstruction(self, layer_id, layer):
        ...
        for model_id, v in self.model_layer_dependencies.items():
            ...
            if all(isinstance(x, layers.Layer) for x in layers):
...
def _finalize_saved_model_layers(layers):
    ...
    for layer in layers:
        ...
        if layer_call and layer_call.concrete_functions:
            call_spec = CallSpec(
                inspect.getfullargspec(layer_call)
            )
            layer.call = utils.use_wrapped_call(
                layer, layer_call, call_spec, return_method=True
            )
            call_has_training_arg = layer._serialized_attributes["metadata"][
                "call_has_training_arg"
            ]
            if "training" in layer_call.function_spec.argument_names:
                # This could change the value of `call_has_training_arg` if this
                # layer doesn't expect a training arg, but has a child layer
                # that does.
                call_has_training_arg = True
            layer._call_spec = CallSpec(inspect.getfullargspec(layer.call))
            if call_has_training_arg is not None:
                layer._call_has_training_arg = call_has_training_arg
        else:
...
def _finalize_metric(metric):
    metric.update_state = types.MethodType(
        utils.update_state_wrapper(metric.keras_api.update_state),
        metric,
    )
...
def revive_custom_object(identifier, metadata):
    ...
    revived_classes = {
        ...
        constants.LAYER_IDENTIFIER: (RevivedLayer, layers.Layer),
        constants.MODEL_IDENTIFIER: (RevivedNetwork, models.Model),
        constants.NETWORK_IDENTIFIER: (
            RevivedNetwork,
            models.Functional,
        ),
...
class RevivedLayer:
    ...
    def _init_from_metadata(cls, metadata):
        ...
        with utils.no_automatic_dependency_tracking_scope(revived_obj):
            ...
            revived_obj._call_has_training_arg = metadata[
                "call_has_training_arg"
            ]
            config = metadata.get("config")
...
def _revive_setter(layer, name, value):
    ...
    if name in PUBLIC_ATTRIBUTES:
        ...
        isinstance(layer, models.Functional)
